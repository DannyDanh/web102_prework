...
function addGamesToPage(games) {
    ...
    for(let i=0; i<games.length; i++){
        ...
        const percentFunded = Math.floor((game.pledged / game.goal) * 100);
        gameCard.innerHTML = `
        <img class="game-img" src="${game.img}" alt="${game.name}">
        <h3>${game.name}</h3>
        <p>${game.description}</p>
        <p><strong>Pledged:</strong> $${game.pledged}</p>
        <p><strong>${percentFunded}% funded</strong></p>
        `;
...
const totalContri = GAMES_JSON.reduce((acc, game) => {return acc + game.backers; },0);
...
function filterUnfundedOnly() {
    ...
    // const listUnfunded = GAMES_JSON.filter(game => game.pledged < game.goal);
...
function filterFundedOnly() {
    ...
    const  listMet = GAMES_JSON.filter((game)=> {return game.pledged >= game.goal;});

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(listMet);
}
...
const displayStr = `A total of $${totalPledged.toLocaleString()} has been raised for ${totalGames} games. Currently, ${numUnfunded} game${numUnfunded === 1 ? "" : "s"} ${numUnfunded === 1 ? "remains" : "remain"} unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container

const descriptionElement = document.createElement("p");
descriptionElement.innerText = displayStr;  // displayStr is your template string
